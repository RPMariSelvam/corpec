...
/**
 * Form Procurement Block.
 *
 * File has been created with `block/create` command. 
 */
class FormProcurementBlock extends PhpBlock
{
    /**
    ...
    public $defaultContectnumberError = 'Please enter a contect number';
    ...
    public $defaultMailSubject = 'Procurement Form submitted on Asalta - An End-to-End Business Automation Software';
...
public function config()
    ...
    public function sendMail($message, $contactnumber, $email, $name, $recaptcha)
    {
        ...
        $html.='</tr>';
        $html.='<tr>';
        $html.='<td style="font-weight: bold;line-height: 2.3;text-align:left;padding: 0 19px">
            Contact Number </td>';
        $html.='<td style="width:50%">'. nl2br(Html::encode($contactnumber)) .'</td>'; 
        $html.='</tr>';
        $html.='<tr>';
        $html.='<td style="font-weight: bold;line-height: 2.3;text-align:left;padding: 0 7%">
            URL </td>';
    ...
    public function getPostResponse()
    {
        ...
        if (Yii::$app->request->isPost) {
            if ($request->post('message') && $request->post('email') && $request->post('name') && $request->post('contactnumber') && $request->post('recaptcha')) {
                $msg = $this->sendMail($request->post('message'), $request->post('contactnumber'), $request->post('email'), $request->post('name'), $request->post('recaptcha'));
                if($msg == 'success'){
                    unset($_POST);
                }
                return $msg;
