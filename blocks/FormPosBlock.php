 ...
 */
class FormPosBlock extends PhpBlock
{
    /**
    ...
    public $defaultMailSubject = 'POS Form submitted on Asalta - An End-to-End Business Automation Software';
    ...
    public function getPostResponse()
    {
        ...
        if (Yii::$app->request->isPost) {
            if ($request->post('message') && $request->post('email') && $request->post('name') && $request->post('contactnumber') && $request->post('recaptcha')) {
               $mes = $this->sendMail($request->post('message'), $request->post('contactnumber'), $request->post('email'), $request->post('name'), $request->post('recaptcha'));
               if($mes == 'success'){
                unset($_POST);
               }
               return $mes;
