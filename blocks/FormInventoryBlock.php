...
/**
 * Form Inventory Block.
 *
 * File has been created with `block/create` command. 
 */
class FormInventoryBlock extends PhpBlock
{
    /**
    ...
    public $defaultMailSubject = 'Inventory Form submitted on Asalta - An End-to-End Business Automation Software';
    /**
     * @inheritDoc
     */
    ...
    public function sendMail($message, $contactnumber, $email, $name)
    {
    ...
    public function getPostResponse()
    {
        ...
        if (Yii::$app->request->isPost) {
            if ($request->post('message') && $request->post('email') && $request->post('name') && $request->post('contactnumber')) {
                $msg = $this->sendMail($request->post('message'), $request->post('contactnumber'), $request->post('email'), $request->post('name'));
                if($msg == 'success'){
                    unset($_POST);
                }
                return $msg;
